...
App.utils = {
    ...
    interfaces: {
        ...
        changeDimension: function (name, flag) {
            var listObj = App.utils.interfaces.getObjByName(name);
            if (listObj) {
                ...
                for (var j = 0; j < listObj.tubesArray.length; j++) {
                    ...
                    listObj.tubesArray[j].visible = listObj.linsArray[j].visible;
                    listObj.linsArray[j].visible = lastV;
                }
                /* sphere enable**/
    ...
    objOfScene.helpers = {
        ...
        init: function (resArr, objectsForConnect, number, d) {
            ...
            if (number.toString().length == 3) {
                ...
                //if (!App.utils.types.webgl.dimensn) {
            ...
            if(webglObj._dimension) {
                ...
                App.utils.types.webgl.camera.lookAt(new THREE.Vector3(0,10,0));
            }
...
App.rebuild2D = {
    buildPlate:function(){
        ...
        texture.needsUpdate = true;
        texture.minFilter = THREE.LinearFilter;
        var mat = metal = new THREE.MeshPhongMaterial({
            color:'#000000',
        ...
        //webglObj._plate.position.x=webglObj._plate.position.y=15;
        webglObj._plate.position.y=5;
        //webglObj._plate.position.z=-1;
        webglObj._plate.rotation.y = Math.PI;
        webglObj.scene.add(webglObj._plate);
        webglObj._plate.visible = false;
    },
    getTexture:function(){
        var canvas = document.createElement('canvas');
...
$(document).ready(function () {
    ...
    $('input:radio').click(function (val) {
        ...
        if (val.currentTarget.value == '3D') {
            ...
            if(listObj.length>0){
                webglObj.camera.lookAt(listObj[listObj.length-1].sphere.position);
            }
            webglObj._plate.visible = false;
        } else {
            webglObj._dimension = false;
            webglObj.camera.lookAt(new THREE.Vector3(0,10,0));
            webglObj.camera.position.set(0, 10, -35);
            webglObj.controls.rotateSpeed = 0.0;
            webglObj.controls.panSpeed = 0.0;
            webglObj.cameraFixed = true;
            webglObj._plate.visible = true;
        }
